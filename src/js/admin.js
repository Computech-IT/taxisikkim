    ...
    async function loadBookings() {
        ...
        try {
            ...
            bookings.forEach(book => {
                ...
                const rideDate = new Date(book.date).toLocaleDateString();

                // Style the status badge
                let badgeClass = 'status-inactive'; // default Pending (gray)
                if (book.status === 'Confirmed') badgeClass = 'status-active'; // green
                
                let badgeStyle = '';
                if (book.status === 'Cancelled') {
                    badgeStyle = 'style="background: rgba(239, 68, 68, 0.1); color: #ef4444; border: 1px solid rgba(239, 68, 68, 0.2);"';
                }
                ...
                tr.innerHTML = `
                    ...
                    <td style="font-size: 0.85rem;">
                        <div><i class="ri-phone-line"></i> ${book.phone}</div>
                        <div style="color: var(--text-dim);"><i class="ri-mail-line"></i> ${book.email}</div>
                    </td>
                    <td style="font-weight: 500;">${book.vehicle_name || 'Deleted Vehicle'}</td>
                    <td style="font-size: 0.85rem;">
                        <span style="color: var(--neon-cyan);">${book.pickup}</span> ➔ <span style="color: var(--neon-purple);">${book.drop_location}</span>
                        <div style="font-size: 0.8rem; color: var(--text-dim); margin-top: 4px;"><i class="ri-calendar-line"></i> Travel Date: ${rideDate}</div>
                    </td>
                    <td style="color: var(--neon-cyan); font-weight: 700;">₹${book.rate}</td>
                    <td>
                        <span class="status-badge ${badgeClass}" ${badgeStyle}>${book.status}</span>
                    </td>
                    <td>
                        <div style="display: flex; gap: 5px;">
                            ${book.status !== 'Confirmed' ? `<button class="btn btn-primary" onclick="updateBookingStatus(${book.id}, 'Confirmed')" style="padding: 5px 8px; font-size: 0.75rem; background: #22c55e; color: #000; border: none; border-radius: 4px; cursor: pointer;">Confirm</button>` : ''}
                            ${book.status !== 'Cancelled' ? `<button class="btn btn-primary" onclick="updateBookingStatus(${book.id}, 'Cancelled')" style="padding: 5px 8px; font-size: 0.75rem; background: #ef4444; color: #fff; border: none; border-radius: 4px; cursor: pointer;">Cancel</button>` : ''}
                            <button class="btn btn-danger" onclick="deleteBooking(${book.id})" style="padding: 5px 8px; font-size: 0.75rem; border-radius: 4px;"><i class="ri-delete-bin-line"></i></button>
                        </div>
                    </td>
                `;
            ...
            bookingsTableBody.innerHTML = '<tr><td colspan="8" style="text-align:center; color: #ef4444;">Error loading bookings.</td></tr>';
        }
    }
