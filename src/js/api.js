...
export const submitBooking = async (payload, { submitBtn, bookingForm, inlineNotify }) => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (inlineNotify) {
                inlineNotify.textContent = 'Success! Redirecting to status page...';
                inlineNotify.className = 'inline-notification success';
            }

            showToast('Success! Redirecting to booking status...');
            bookingForm.reset();

            // Redirect to status page
            window.location.href = `/booking-status.html?id=${data.bookingId}`;
        } else {
